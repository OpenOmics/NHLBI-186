...
Here we use the TMM-normalized data as the input for maSigPro, following the approach maSigPro authors used for their paper (https://academic.oup.com/bioinformatics/article/30/18/2598/2475510). 
Lowly expressed genes are removed in the same way as we did in DEG analysis
...
cat(paste0(nrow(y),' genes are retained after filtering'))

# TMM method to normalize expression table by samples to account for different sequencing depth and composition between samples
# cpm is calculated
exps.msp <-y %>%
  calcNormFactors(.,method = 'TMM') %>%
  cpm(log = F,normalized.lib.sizes = T)

try(if(!all(names(exps.msp) == rownames(pheno))) stop("pheno data and expression data does not match each other"))

plotDensities(log2(exps.msp +1), 
              group = groups, 
              legend = 'topright',
              main = paste0(setLabel, "remove lowly expressed gene + TMM + CPM normalization"))
...
Patients are used as replicates in this analysis. We used a quadratic regression model to allow some curvatures. As maSigPro does not perform normalization, we used the TMM normalized matrix as input.
...
try(if(!all(names(exps.msp) == rownames(design))) stop("pheno data and expression data does not match each other"))
# find genes with significant change over time
fit <- p.vector(data = exps.msp, # normalized data matrix
...
```{r show acat1}
acat1 <- exps.msp[rownames(exps.msp)=="ACAT1", ]

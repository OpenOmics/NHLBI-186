...
plotDensities(exps.deseq2, group = groups, legend = 'topright',main = paste0(setLabel, " after filtering and vst Normalization"))
boxplot(value~X2, las=2, ylab = "Counts", xlab = NULL, cex.axis=0.75,
        melt(exps.deseq2), # melt just aggregate values in each column into a single colum and remove row names (using the sample names as row names instead)
        main=paste0(setLabel, " After Filtering and vst normalization"), col=groupcol)
...
try(if(!all(names(exps.deseq2) == rownames(design))) stop("pheno data and expression data does not match each other"))
...
fit <- p.vector(data = exps.deseq2, # normalized data matrix
                ...
                counts = T, # data is RNA seq so set it as True, GLM is used. if it is microarray data or following gaussian distribution, set it to false; here we use variance stabilized data, which follows gaussian distribution, so we use counts=F)
                )
```
